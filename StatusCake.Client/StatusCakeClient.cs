  ...
  public class StatusCakeClient : IStatusCakeClient, IDisposable
  {
    ...
    public async Task<PutResponse> UpdateOrCreateTestAsync(TestUpdate test)
    {
      ...
      if (Validator.TryValidateObject(test, context, results))
      {
        ...
        var dataParameters = new NameValueCollection
                {
                  {"BasicPass", test.BasicPass },
                  {"BasicUser", test.BasicUser },
                  ...
                  {"DNSIP", test.DNSIP },
                  {"DNSServer", test.DNSServer },
                  {"DoNotFind", test.DoNotFind == null ? null : (test.DoNotFind.Value ? "1" : "0")},
                  {"EnableSSLAlert", test.EnableSSLAlert == null ? null : (test.EnableSSLAlert.Value ? "1" : "0")},
                  {"FindString", test.FindString},
                  {"FollowRedirect", test.FollowRedirect == null ? null : (test.FollowRedirect.Value ? "1" : "0")},
                  {"IncludeHeader", test.IncludeHeader == null ? null : (test.IncludeHeader.Value ? "1" : "0")},
                  ...
                  {"Port", test.Port == null ? null : test.Port.ToString()},
                  {"PostBody", test.PostBody },
                  {"PostRaw", test.PostRaw },
                  {"Public", test.PublicReporting == null ? null : (test.PublicReporting.Value ? "1" : "0")},
                  {"RealBrowser", test.TestWithRealBrowser == null ? null : (test.TestWithRealBrowser.Value ? "1" : "0")},
                  {"StatusCodes", test.StatusCodes != null ? string.Join(",", test.StatusCodes) : null},
                  {"TestID", test.TestID > 0 ? test.TestID.ToString() : null},
                  {"TestTags", test.TestTags != null ? string.Join(",", test.TestTags) : null},
                  {"TestType", test.TestType.ToString()},
                  {"Timeout", test.Timeout?.ToString()},
                  {"TriggerRate", test.TriggerRate?.ToString()},
                  {"UseJar", test.UseJar == null ? null : (test.UseJar.Value ? "1" : "0")},
